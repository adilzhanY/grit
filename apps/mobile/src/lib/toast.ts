/**
 * Tiny toast bus. Store log actions call `logToast(...)`; the <Toast /> overlay
 * subscribes via `onToast`. Kept outside React state so any log entry point
 * fires it with zero wiring. Mirrors apps/web/src/lib/toast.ts.
 */
